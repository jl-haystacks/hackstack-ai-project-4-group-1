...
df = pd.read_csv('Data/GA_LISTINGS_SALES_V2.csv')

# Define features to be utilized for generating scatter plots
features = ['beds', 'baths_full', 'baths_half', 'square_footage', 'lot_size', 'year_built']
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        # Section for displaying SHAP values
        html.Div([
            html.H1(
                children="SHAP Values here",
                ),
...
def update_graph(feature, model, gradient):
    ...
    hover_names = []
    for ix, val in zip(df.iloc[:,0].values, df[feature].values):
